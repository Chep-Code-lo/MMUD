// ============================================================
//  des.cpp — DES Algorithm (thuan C++, khong can thu vien)
// ============================================================
#include "des.h"
#include "config.h"
...
KeySchedule buildKeySchedule(uint8_t key[8]) {
    KeySchedule ks;
    uint64_t keyVal = 0;
    for (int i = 0; i < 8; i++) keyVal = (keyVal << 8) | key[i];

    uint64_t CD = 0;
    for (int i = 0; i < 56; i++) CD = (CD << 1) | getBit64(keyVal, PC1[i]);
    ...
    for (int round = 0; round < 16; round++) {
        ...
        uint64_t subkey = 0;
        for (int i = 0; i < 48; i++)
            subkey = (subkey << 1) | ((CD56 >> (56 - PC2[i])) & 1);
        ks.K[round] = subkey;
    }
    return ks;
}

uint64_t desEncrypt(uint64_t plain, const KeySchedule& ks) {
    uint64_t perm = 0;
    for (int i = 0; i < 64; i++) perm = (perm << 1) | getBit64(plain, IP[i]);
    ...
    for (int round = 0; round < 16; round++) {
        uint64_t ER = 0;
        for (int i = 0; i < 48; i++) ER = (ER << 1) | getBit32(R, E_TABLE[i]);
        uint64_t xored = ER ^ ks.K[round];
        ...
        for (int s = 0; s < 8; s++) {
            ...
            int col   = (chunk >> 1) & 0xF;
            sout = (sout << 4) | SBOX[s][row][col];
        }

        uint32_t Pout = 0;
        for (int i = 0; i < 32; i++) Pout = (Pout << 1) | getBit32(sout, P_TABLE[i]);
    ...
    uint64_t cipher = 0;
    for (int i = 0; i < 64; i++) cipher = (cipher << 1) | getBit64(preout, IP_INV[i]);
    return cipher;
}
